...
# Defaults
DEFAULT_ADD_FIREWALL = True
DEFAULT_N_IPS = 1
DEFAULT_PROTOCOL = 'TCP'
DEFAULT_SRC_RANGES = ['0.0.0.0/0']
...
def GenerateGlobalForwardingRule(context, number):
  ...
  name = context.env['name']
  prop = context.properties
  protocol = prop.setdefault(IP_PROTO, DEFAULT_PROTOCOL)
  # pyformat: disable
  return {
...
def GenerateNGlobalForwardingRules(context):
  ...
  prop = context.properties
  n_of_ips = prop.setdefault(N_OF_IPS, DEFAULT_N_IPS)
...
def GenerateFirewall(context):
  ...
  name = context.env['name']
  prop = context.properties
  src_ranges = prop.setdefault(SRC_RANGES, DEFAULT_SRC_RANGES)
  add_firewall = prop.setdefault(ADD_FIREWALL, DEFAULT_ADD_FIREWALL)
  instances = []
  if add_firewall:
    # pyformat: disable
    instances.append({
