...
# Defaults - Also check default of the autoscaled_group
DEFAULT_HEALTH_PATH = '/_ah/health'
DEFAULT_SERVICE_NAME = 'httpservice'
DEFAULT_PORT = 80
DEFAULT_REPLICAS = [{'pri': {'zone': 'us-central1-f'}}]


def GenerateBackendService(context):
  """Generates one backendService resource."""
  prop = context.properties
  port = prop.setdefault(default.PORT, DEFAULT_PORT)
  health_path = prop.setdefault(default.HEALTH_PATH, DEFAULT_HEALTH_PATH)
  default_srv = prop.setdefault(default.SERVICE, DEFAULT_SERVICE_NAME)
...
def GenerateBackends(context):
  ...
  prop = context.properties
  replicas = prop.setdefault(REPLICAS, DEFAULT_REPLICAS)
  backends = []
  for _, zone_dict in common.OrderedItems(replicas):
